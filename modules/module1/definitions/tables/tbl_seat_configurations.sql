CREATE TABLE seat_configurations (
    ...
    CONSTRAINT PK_seat_configurations
        PRIMARY KEY (id),

    CONSTRAINT FK_seat_configurations_type
        FOREIGN KEY (aircraft_type_id)
        REFERENCES aircraft_types(id),

    CONSTRAINT UQ_seat_configurations_name
        UNIQUE (aircraft_type_id, name),

    CONSTRAINT UQ_seat_configurations_id_type
        UNIQUE (id, aircraft_type_id),

    CONSTRAINT CK_seat_config_economy
        CHECK (economy_seats >= 0),

    CONSTRAINT CK_seat_config_business
        CHECK (business_seats >= 0),

    CONSTRAINT CK_seat_config_total
        CHECK (economy_seats + business_seats > 0)
);
